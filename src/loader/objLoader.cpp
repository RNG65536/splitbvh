...
static bool calcTangentVectors(
    const gVec3& v1,
    const gVec3& v2,
    const gVec3& v3,
    const gVec2& w1,
...
static void generateTangentFrames(const std::vector<gVec3>& vertices, const std::vector<gVec2>& uvs, const std::vector<gVec3>& normals, const std::vector<gInt3>& indices,
    std::vector<gVec3>& out_tangents, std::vector<gVec3>& out_bitangents);

static gVec3 calcNormal(gVec3 v0, gVec3 v1, gVec3 v2);

static void generateNormals(
    std::vector<gInt3> &faces,
...
void generateNormals(std::vector<gInt3> &faces, std::vector<gVec3> &vertices, std::vector<gVec3> &normals)
{
...
bool calcTangentVectors(const gVec3& v1, const gVec3& v2, const gVec3& v3, const gVec2& w1, const gVec2& w2, const gVec2& w3, gVec3& sdir, gVec3& tdir)
{
    double x1 = v2.x - v1.x;
    double x2 = v3.x - v1.x;
    double y1 = v2.y - v1.y;
    ...
    sdir = gVec3((t2 * x1 - t1 * x2) * r, (t2 * y1 - t1 * y2) * r,
        (t2 * z1 - t1 * z2) * r);
    tdir = gVec3((s1 * x2 - s2 * x1) * r, (s1 * y2 - s2 * y1) * r,
        (s1 * z2 - s2 * z1) * r);
...
void generateTangentFrames(const std::vector<gVec3>& vertices, const std::vector<gVec2>& uvs, const std::vector<gVec3>& normals, const std::vector<gInt3>& indices, std::vector<gVec3>& out_tangents, std::vector<gVec3>& out_bitangents)
{
    ...
    std::vector<gVec3> temp_tangents(vertices.size(), gVec3(0, 0, 0)); // vertex tangent
    std::vector<gVec3> temp_bitangents(vertices.size(), gVec3(0, 0, 0)); // vertex bitangent
        ...
        if (!calcTangentVectors(v1, v2, v3, w1, w2, w3, sdir, tdir)) {
            ...
            if (!calcTangentVectors(v2, v3, v1, w2, w3, w1, sdir, tdir)) {
                ...
                if (!calcTangentVectors(v3, v1, v2, w3, w1, w2, sdir, tdir)) {
                    //                         lprintf("(%d) ---------- 312 not ok!\n", n);

                    //                         lprintf("\tdegenerate uv/tangent space!\n");
                    n_degen_tan++;
    ...
    if (n_degen_tan > 0)
    {
        Logger::info() << "generate_tangents :: degen count: " << n_degen_tan << std::endl;
    }

    //it was for face normal, now project the tangent vectors to align with vertex normals
    for (int a = 0; a < uvs.size(); a++)
    {
        ...
        if (dot(temp_bitangents[a], bitangent2) >= 0) bitangent2 *= -1; //bitangent uses n, while bitangent2 uses -n,
        ...
        // tangent[a].w = (Dot(Cross(n, t), tan2[a]) < 0.0F) ? -1.0F : 1.0F; //handedness
    }
}
...
bool ObjLoader::loadFromObjFile(const char* filename, const char* basepath)
{
    ...
    Logger::info() << "Loading " << filename << std::endl;
    std::string err;
    std::string path = std::string(basepath) + std::string(filename);
    if (!tinyobj::LoadObj(&attrib, &shapes, &materials, &err, path.c_str(), basepath))
    {
        Logger::info() << "error: " << err << std::endl;
        return false;
    }
...
ObjLoader::ObjLoader(const char* basepath, const char* filename, bool rescaled /*= false*/)
{
    m_rescaled = rescaled;
...
void ObjLoader::convertVertices(const tinyobj::attrib_t& obj_attribs, const std::vector<tinyobj::shape_t> &obj_shapes)
{
    m_faces.clear();
    ...
    {
        char buf[256];
        sprintf_s(buf, "%d %d %d", i.vertex_index, i.texcoord_index, i.normal_index);
    ...
    for (size_t i = 0; i < all_verts.size() / 3; i++)
    {
        ...
        m_faces.push_back(gInt3(
            vertex_lut[hash_vert(a)],
    ...
    for (size_t i = 0; i < unique_verts.size(); i++)
    {
        ...
        if (vt >= 0)
        {
            m_uvs.push_back(gVec2(
                obj_attribs.texcoords[vt * 2 + 0],
                obj_attribs.texcoords[vt * 2 + 1]));
        }

        if (vn >= 0)
        {
            m_normals.push_back(gVec3(
                obj_attribs.normals[vn * 3 + 0],
...
void ObjLoader::materialBatching()
{
    ...
    std::sort(sort_info.begin(), sort_info.end(), [](face_mat_pair& a, face_mat_pair&b){
    ...
    if (n_materials > 0)
    {
        ...
        {
            int n_mat_face = std::count_if(sort_info.begin(), sort_info.end(), [n](face_mat_pair _n)
            {
                return _n.second == n;
            });
            ...
            {
                Logger::info() << "unused material " << n << " !!!!!!!!!!!!!!!!!" << std::endl;
                m_sorted_shape_offset.push_back(temp_offset);
...
void ObjLoader::convertFromTinyobjMesh(const tinyobj::attrib_t& obj_attribs, const std::vector<tinyobj::shape_t>& obj_shapes, const std::vector<tinyobj::material_t>& obj_materials)
{
    ...
    if (is_gen_tangents && m_uvs.size() > 0)
    {
        ...
        generateTangentFrames(m_vertices, m_uvs, m_normals, m_faces, m_tangents, m_bitangents);
    }
    else
    {
    ...
    for (int i = 0; i < obj_materials.size(); i++)
    {
        ...
        mat.Ka = gVec3(obj_materials[i].ambient[0], obj_materials[i].ambient[1], obj_materials[i].ambient[2]);
        mat.Kd = gVec3(obj_materials[i].diffuse[0], obj_materials[i].diffuse[1], obj_materials[i].diffuse[2]);
        mat.Ks = gVec3(obj_materials[i].specular[0], obj_materials[i].specular[1], obj_materials[i].specular[2]);
...
void ObjLoader::info(const std::vector<tinyobj::shape_t>& shapes, const std::vector<tinyobj::material_t>& materials)
{
    Logger::info() << "============================= info =============================" << std::endl;
    Logger::info() << "faces      : " << m_faces.size() << std::endl;
    Logger::info() << "vertices   : " << m_vertices.size() << std::endl;
    Logger::info() << "uvs        : " << m_uvs.size() << std::endl;
    Logger::info() << "normals    : " << m_normals.size() << std::endl;
    Logger::info() << "mat_ids    : " << m_material_ids.size() << std::endl;
    Logger::info() << "tangents   : " << m_tangents.size() << std::endl;
    Logger::info() << "bitangents : " << m_bitangents.size() << std::endl;
    Logger::info() << std::endl;

    Logger::info() << "obj shapes : " << shapes.size() << std::endl;

    int sid = 0;
    for (auto s : shapes)
    {
        Logger::info() << "    shape " << sid++ << " <" << s.name
                       << "> : index(" << s.mesh.indices.size() / 3 << "), mat("
                       << s.mesh.material_ids.size() << ")" << std::endl << std::endl;
    }

    Logger::info() << "obj materials : " << materials.size() << std::endl;
}
