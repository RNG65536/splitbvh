...
bool SAHBVHLeafNode::intersect(const Ray& ray, float& tNear) const
{
    tNear = kInfinity;
    ...
    for (int i = 0; i < k_leaf_node_size; i++)
    {
        ...
        if (hit)
        {
            ...
            assert(t_near > ray.tmin() && t_near < ray.tmax());
...
bool SAHBVHBranchNode::intersect(const Ray& ray, float& tNear) const
{
    Logger::debug() << "[ " << debug_level << " ][ " << debug_tag << " ]"
                    << std::endl;
...
float MedianSplitter::cost() const { return m_cost; }
...
std::shared_ptr<SAHBVHNode> SAHBVH::buildLeaf(int                begin,
                                              int                end,
                                              uint32_t           level,
...
bool SAHBVH::intersect(const Ray& ray, float& tNear) const
{
    // BVH traversal
    tNear = kInfinity;
