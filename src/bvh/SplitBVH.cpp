...
bool SplitBVHLeafNode::intersect(const Ray& ray, float& tNear) const
{
    tNear = kInfinity;
    ...
    for (int i = 0; i < k_sbvh_leaf_node_size; i++)
    {
        ...
        bool  hit    = primitive[i]->intersect(ray, t_near);
        if (hit)
        {
#if DEBUG_INFO
...
bool SplitBVHBranchNode::intersect(const Ray& ray, float& tNear) const
{
    Logger::debug() << "[ " << debug_level << " ][ " << debug_tag << " ]"
                    << std::endl;
    ...
    float ray_d[3] = {ray.direction().x, ray.direction().y, ray.direction().z};
    ...
    if (ray_d[split_axis] > 0.0f)
    {
        ...
        {
            float t;
            bool  hit = left_child->intersect(ray, t);
            if (hit)
            {
        ...
        {
            float t;
            bool  hit = right_child->intersect(ray, t);
            if (hit)
            {
                t_near = std::min(t_near, t);
                anyHit = true;
...
float MedianSplitBVHSplitter::cost() const { return m_cost; }
    ...
    : m_triangles(triangles), m_node_bound(node_bound)
{
    ...
    int   debug_x_count[k_sbvh_num_binning_buckets];
    int   debug_y_count[k_sbvh_num_binning_buckets];
    int   debug_z_count[k_sbvh_num_binning_buckets];
    float debug_x_min_cost = kInfinity;
    float debug_y_min_cost = kInfinity;
    float debug_z_min_cost = kInfinity;
#endif
    ...
    if (edge.x > k_edge_split_threshold)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            int bucket_index =
                std::min(int((x0 - node_bound.min().x) / delta_x),
...
////////////// debug ////////////////
    ...
    if (edge.y > k_edge_split_threshold)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            int bucket_index =
                std::min(int((y0 - node_bound.min().y) / delta_y),
                         k_sbvh_num_binning_buckets - 1);
...
////////////// debug ////////////////
    ...
    if (edge.z > k_edge_split_threshold)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            int bucket_index =
                std::min(int((z0 - node_bound.min().z) / delta_z),
                         k_sbvh_num_binning_buckets - 1);
...
float SAHBVHSplitter::cost() const { return m_min_cost; }
    ...
    std::vector<std::shared_ptr<BoundedTriangle>>& right_branch)
{
    ...
    // average performance is better but slower in extreme cases
    ...
    for (int i = 0; i < total; i++)
    {
        ...
        int         bucket_index =
            std::min(int((min_v[m_min_axis] - node_min_v[m_min_axis]) / delta),
                     k_sbvh_num_binning_buckets - 1);
    ...
    : m_triangles(triangles), m_node_bound(node_bound)
{
    ...
    AABB  debug_y_aabb[k_sbvh_num_binning_buckets];
    AABB  debug_z_aabb[k_sbvh_num_binning_buckets];
    int   debug_x_entries[k_sbvh_num_binning_buckets];
    int   debug_y_entries[k_sbvh_num_binning_buckets];
    int   debug_z_entries[k_sbvh_num_binning_buckets];
    int   debug_x_exits[k_sbvh_num_binning_buckets];
    int   debug_y_exits[k_sbvh_num_binning_buckets];
    ...
    if (edge.x > k_edge_split_threshold)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            assert(x0 >= node_min);
            assert(x0 <= node_max);
            int i0 = std::min(int((x0 - node_min) / delta_x),
                              k_sbvh_num_binning_buckets - 1);
            ...
            assert(x1 >= node_min);
            assert(x1 <= node_max);
            int i1 = std::min(int((x1 - node_min) / delta_x),
                              k_sbvh_num_binning_buckets - 1);
...
#if 1  // TODO : clip bounded triangle not bounding box (complicated)
            clipped_bbox =
...
#else
            // clipAndExpand inlined
            bins[i0].m_num_entries++;
            clipped_bbox = box.clipX(bin_min, bin_min + delta_x);
            bins[i0].m_bounding_box.expandBy(clipped_bbox);
            bins[i0].m_is_valid = 1;
#endif
...
////////////// debug ////////////////
        ...
        for (int i = 1; i < k_sbvh_num_binning_buckets; i++)
        {
            ...
            for (int j = 0; j < i; j++)
            {
                // if (bins[j].m_num_entries > 0)
                //{
                //    count_left += bins[j].m_num_entries;
                //}

                // if (bins[j].m_bounding_box.isValid())
                //{
                //    bound_left.expandBy(bins[j].m_bounding_box);
                //}
    ...
    if (edge.y > k_edge_split_threshold)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            assert(y0 >= node_min);
            assert(y0 <= node_max);
            int i0 = std::min(int((y0 - node_min) / delta_y),
                              k_sbvh_num_binning_buckets - 1);
            assert(i0 < k_sbvh_num_binning_buckets);
            assert(i0 >= 0);
            ...
            assert(y1 >= node_min);
            assert(y1 <= node_max);
            int i1 = std::min(int((y1 - node_min) / delta_y),
                              k_sbvh_num_binning_buckets - 1);
            assert(i1 < k_sbvh_num_binning_buckets);
            assert(i1 >= 0);
...
#else
            bins[i0].m_num_entries++;
            clipped_bbox = box.clipY(bin_min, bin_min + delta_y);
            bins[i0].m_bounding_box.expandBy(clipped_bbox);
            bins[i0].m_is_valid = 1;
#endif
            // update middle bins
...
float SplitBVHSplitter::cost() const { return m_min_cost; }
    ...
    std::vector<std::shared_ptr<BoundedTriangle>>& right_branch)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        assert(v0 >= node_min);
        assert(v0 <= node_max);
        int i0 = std::min(int((v0 - node_min) / delta),
                          k_sbvh_num_binning_buckets - 1);
        assert(i0 < k_sbvh_num_binning_buckets);
        assert(i0 >= 0);
        ...
        assert(v1 >= node_min);
        assert(v1 <= node_max);
        int i1 = std::min(int((v1 - node_min) / delta),
                          k_sbvh_num_binning_buckets - 1);
        assert(i1 < k_sbvh_num_binning_buckets);
        assert(i1 >= 0);
...
#else
            AABB clipped_bbox = box.clipAxis(node_min, node_clip, m_min_axis);
            left_branch.push_back(std::make_shared<BoundedTriangle>(
                m_triangles[i], clipped_bbox));
#endif
        }
...
#else
            AABB clipped_bbox = box.clipAxis(node_clip, node_max, m_min_axis);
            right_branch.push_back(std::make_shared<BoundedTriangle>(
                m_triangles[i], clipped_bbox));
#endif
        }
...
SplitBVH::SplitBVH(const Mesh& mesh)
{
    m_triangles.clear();
...
#else
        float spatial_split_cost = kInfinity;
#endif
...
bool SplitBVH::intersect(const Ray& ray, float& tNear) const
{
    // BVH traversal
    tNear = kInfinity;
